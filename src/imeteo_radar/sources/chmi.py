...
class CHMIRadarSource(RadarSource):
    ...
    def __init__(self):
        super().__init__("chmi")
        self.base_url = "https://opendata.chmi.cz/meteorology/weather/radar/composite/maxz/hdf5"
    ...
    def _download_single_file(self, timestamp: str, product: str) -> Dict[str, Any]:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(suffix=f'_chmi_{product}_{timestamp}.hdf', delete=False) as temp_file:
    ...
    def download_latest(self, count: int, products: List[str] = None,
        ...
        print(f"📋 CHMI: Downloaded {len(downloaded_files)} files ({len(download_tasks)-len(downloaded_files)} failed)")
        ...
        try:
            with h5py.File(file_path, 'r') as f:
                ...
                if 'LL_lon' in where_attrs and 'UR_lon' in where_attrs:
                    ...
                    print("⚠️  Corner coordinates not found in HDF5, using approximate extent")
                ...
                return {
                    ...
                    'timestamp': timestamp[:14] if len(timestamp) >= 14 else timestamp  # YYYYMMDDHHMMSS format
                }
    ...
    def cleanup_temp_files(self):
        ...
        if cleaned_count > 0:
            print(f"🧹 Cleaned up {cleaned_count} temporary CHMI files")
